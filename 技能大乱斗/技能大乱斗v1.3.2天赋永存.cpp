...
void save()
{
	...
	while (fin >> sa)
	{
		GFS = sa % 10;
		dGFS = (sa % 100 - GFS) / 10;
		c = (sa % 10000000 - dGFS * 10 - GFS) / 100;
		lv = (sa - c * 100 - dGFS * 10 - GFS) / 10000000;
	}
	if (c >= 2)
	{
		p = c % 2;
...
int main()
{
	...
	Sleep(5000);
	system("cls");
	cout << "v1.3.2天赋永存" << endl << "更新说明:" << endl << "1.开放了天赋技能系统的存档" << endl << "2.修复了一些已知bug" << endl;
...
choose:cout << "段位：";
	...
	if (q == 4)
	{
		sa = 100 * c + 200 * b + 400 * a + 800 * s + 1600 * ss;
		for (int i = 4; i <= 33; i++)
		{
			if (4 <= i <= 6 && ji[i] == 1)sa += 100;
