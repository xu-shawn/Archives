...
#include<conio.h>
using namespace std;
long long q,p,m,ji[10001]={0},dji[10001]={0},hp=1000,dhp=1000,pg=100,dpg=100,pw,dpw,c=1,b=0,a=0,s=0,ss=0,dc=0,db=0,da=0,ds=0,dss=0,u=3,o,r=1,l,www=0,god=0,lv=1,sa=1,d=1,win=0,lose=0,jie,e=0,du=0,GFS,dGFS,tz=1,atk=0,wake=1,need=1,have=0;
...
void save()
{
    ...
    while(fin>>sa)
	{
		wake=sa%10;
		GFS=(sa%100-wake)/10;
		dGFS=(sa%1000-GFS*10-wake)/100;
		c=(sa%100000000-dGFS*100-GFS*10-wake)/1000;
		lv=(sa-c*1000-dGFS*100-GFS*10-wake)/100000000;
	}
    if(c>=2)
	{
		p=c%2;
...
int main()
{
	...
	Sleep(5000);
	system("cls");
	cout<<"v1.3.6天赋觉醒"<<endl<<"更新说明:"<<endl<<"1.开放了天赋技能的升级系统"<<endl<<"2.修复了一些已知bug"<<endl;
	...
	if(q==4)
	{
		...
		sa+=GFS*10;
		sa+=dGFS*100;
		sa+=1000*c+2000*b+4000*a+8000*s+16000*ss;
		for(int i=4;i<=33;i++)
		{
			if(4<=i<=6&&ji[i]==1)sa+=1000;
			if(7<=i<=14&&ji[i]==1)sa+=2000;
			if(15<=i<=22&&ji[i]==1)sa+=4000;
			if(23<=i<=26&&ji[i]==1)sa+=8000;
			if(27<=i<=33&&ji[i]==1)sa+=16000;
		}
		sa+=lv*100000000;
